...
<!-- Реализован трекер на основе Венского алгоритма и IoU.
Проблема трекера в том, что когда объект не распознается и уходит слишком далеко, его IoU с самим собой в момент последней детекции равен 0, а когда другой объект проходит рядом с этим местом последней детекции, трекер думает, что нашелся пропавший объект и дает ему некорректный track_id -->
SORT IoU